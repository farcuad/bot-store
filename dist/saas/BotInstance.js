...
export class BotInstance extends EventEmitter {
    ...
    async _handleMessage(msg) {
        try {
            ...
            const sys = (key) => config.respuestas_sistema[key]?.texto ?? "";
            ...
            else if (nowInSeconds - session.last_interaction >
            ...
            // ── Generación IA ────────────────────────────────────────────────────────
            try {
                let respuesta = await generarRespuestaBot(activeSession.history, this.configSvc.getNombre(), config.respuestas_info, instruccionExtra, config.prompt_ia);
