...
async function fetchHorario() {
    ...
    if (!doc.exists) {
        console.warn("⚠️ No se encontró horarios/atencion en Firestore. Usando valores por defecto.");
...
export async function loadConfig() {
    try {
        const [{ nombre, activo }, respuestas_info, respuestas_sistema, horario] = await Promise.all([
            fetchNombreYActivo(),
            fetchInfoRespuestas(),
        ...
        console.log(`🔥 Config cargada desde Firestore — "${nombre}" | ` +
...
export function isWithinBusinessHours() {
    const { dias_habiles, hora_apertura, hora_cierre, timezone } = getConfig().horario;
 ...
 */
export async function registrarNoEntendido(mensaje, usuarioId, nombreUsuario) {
    try {
