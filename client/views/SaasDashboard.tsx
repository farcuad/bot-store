...
const SaasDashboard: React.FC = () => {
  ...
  const [dashboardTab, setDashboardTab] = useState<'bots' | 'plans'>('bots');
    ...
    <div className="max-w-6xl mx-auto">
      {isAdmin && (
        <div className="flex gap-4 mb-8 border-b border-white/10 pb-4">
          <button 
            onClick={() => setDashboardTab('bots')}
            className={`px-4 py-2 font-bold rounded-lg transition-colors ${dashboardTab === 'bots' ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-white'}`}
          >
            Mis Bots
          </button>
          <button 
            onClick={() => setDashboardTab('plans')}
            className={`px-4 py-2 font-bold rounded-lg transition-colors ${dashboardTab === 'plans' ? 'bg-white/10 text-white' : 'text-gray-500 hover:text-white'}`}
          >
            Planes
          </button>
        </div>
  ...
  return (
    <div className="bg-[#12121a] p-6 rounded-2xl border border-white/10">
      ...
      <p className="text-sm text-gray-400 mb-4">
        Edita la estructura JSON de los planes que se muestran en la Landing Page.
      </p>
      <textarea 
